...
public class Item implements Serializable{   //ftwwwwwwww future implentation for byte array <3 <3
    ...
    public boolean isReorderNeeded() {
        try {
            int qty = quantity; // Convert quantity to int
            int threshold = 10; // Set your threshold here
            return qty < threshold; // Return true if reorder is needed
        } catch (NumberFormatException e) {
            System.out.println("Invalid quantity format for item: " + itemId);
            return false; // Handle invalid quantity, assume reorder is not needed
        }
    }
