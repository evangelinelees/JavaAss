...
//        writeToLog(loggedInUser," | Item added | ","SUCCESS");
   ...
   @Override
    ...
    public void deductItemQuantity(String itemId, int losses, int sold) {
        List<Item> items = loadAll(); // Load all items
        boolean updated = false;

        for (Item item : items) {
            if (item.getItemId().equals(itemId)) {
                int newQuantity = item.getQuantity() - (losses + sold);
                if (newQuantity < 0) {
                    throw new IllegalArgumentException("Quantity cannot be negative after deduction.");
                }
    ...
    public void updateItemQuantity(Item item) {
        List<Item> items = loadAll();  // Load all items from the storage
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getItemName().equalsIgnoreCase(item.getItemName())) {
                items.set(i, item);  // Update the item in the list
                saveAll(items);  // Save the updated list of items to storage (file/database)
                return;
            }
        }
        // If item not found, no need for a separate action
    }
